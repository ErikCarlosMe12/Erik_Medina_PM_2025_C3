...
void main(void)
{
    ...
    printf("\nX = %d \t Y = %d \t Z[0] = %d \tZ[1] = %d \t [3] = 5%d \tZ[4]= %d", X, Y,
           Z[0], Z[1], Z[2], Z[3], Z[4]);

 int *IX; /* IX representa un apuntado a un entero. */
...
IX = &Z[2];   /* IX apunta el tercer elemento del arreglo Z. */
Y = *IX;     /* Y toma el valor de Z[2], ahora vale 6. */
*IX = 12;    /* Z[2] se modifica, ahora vale 15. */
printf("\nX = %d \t Y = %d \T Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);


++*IX;   /* Z2,, se modific. El apuntador IX accede temporalmente a Z3, por
lo  tanto X toma este valor (8). Observa que IX no se reasigna */
...
Y = *IX;   /* Y semodifica, ahora vale Z[3] (8). */
printf("\nX = %d \t Y = %d \t Z[0]= %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);


 IX = IX + 4; /* IX se modifica. Observa que el apuntador se mueve 4 posiciones
 y cae en una direccion afuera del arreglo. Esto ocasionara un error
