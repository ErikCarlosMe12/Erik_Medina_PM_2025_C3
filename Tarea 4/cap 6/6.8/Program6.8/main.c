#include <stdio.h>

/* Traspuesta.
El programa calcula la traspuesta de una matriz. */

#define MAX 50

void Lectura(int [][MAX], int, int);
void Traspuesta(int [][MAX], int [][MAX], int, int);
void Imprime(int [][MAX], int, int);


int main(void)
{
    int MAT[MAX][MAX], TRA[MAX][MAX];
    int FIL, COL;

    do {
            printf("Ingrese el numero de filas de la matriz: ");
            scanf("%d", &FIL);

            } while (FIL > MAX || FIL < 1); /* Verifica filas */

            do {

                printf("Ingrese el numero de columnas de la matris: ");
                scanf("%d", &COL);

                } while (COL > MAX || COL < 1); /* Verifica columnas*/

                Lectura(MAT, FIL, COL);
                Traspuesta(MAT, TRA, FIL, COL);
                Imprime(TRA, COL, FIL);
                return 0;

            }


    void Lectura(int A[][MAX], int F, int C)
    /* Lee una matriz de tipo entero de F filas y C columas. */
    {
        int I, J;
        for (I = 0; I < F; I++)
        for (J = 0; J < C; J++) {
            printf("Ingrese el elemento %d %d: ", I + 1, J + 1);
            scanf("%d", &A[I][J]);

        }
    }
    void Traspuesta(int M1[][MAX], int M2[][MAX], int F, int C)
    /* Calcula la traspuesta: M2 = M*T. */
    {
        int I, J;
        for (I = 0; I < F; I++)
            for (J = 0; J < C; J++)
            M2[J][I] = M1[I][J];

    }

    void Imprime(int A[][MAX], int F, int C)
    /*Escribe una matriz de F filas por C columnas. */

    {
        int I, J;
        for (I = 0; I < F; I++)
            for (J = 0; J < C; J++)
                printf("Elemento %d %d: %d\n", I + 1, J + 1, A[I][J]);

    }



