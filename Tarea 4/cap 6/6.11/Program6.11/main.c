...
int main(void)
{
    ...
    /* REN almacena el rendimiento anual de los fondos. */
...
/* Lee un arreglo F x C de reales. */
{
    int I, J;
        for (I = 0; I < F; I++)
            for (J = 0; J < C; J++) {
...
/* Calcula el rendimiento anual de cada fondo y lo guarda en V (%%). */
{
    int I;
...
/* Calcula el promedio anualizado por fondo. */
{

    int I, J;
    float SUM, PRO;
    printf("\n\nPromedio anualizado de la acciones de los fondos");
    for (I = 0; I < F; I++) {   /* <-- antes estaba R, debe ser F */
        SUM = 0.0f;
        for (J = 0; J < C; J++)
            SUM += A[I][J];
        PRO = SUM / C;
...
/* Imprime el mejor y peor rendimiento. */
{
        ...
        for (I = 1; I < F; I++){
            if (A[I] > ME) { ME = A[I]; M = I;}
            if (A[I] < PE) { PE = A[I]; P = I;}
        }
